...
// Dashboard admin, yaitu tampilan pengawasan.
//
// Admin berperan sebagai manager di aplikasi ini: dialah yang menugaskan
// konten dan yang perlu tahu kondisi tim secara keseluruhan. Karena itu
// halaman ini menjawab pertanyaan seorang manager, bukan pertanyaan seorang
// pengeksekusi: siapa sedang memegang berapa, apa yang belum ada pemiliknya,
// dan apa yang tersendat.
//
// Staff melihat halaman yang berbeda, lihat DashboardStaff.jsx.
//
...
export default function DashboardAdmin() {
  const { user } = useAuth()
  // Wallpaper diambil dari TenantContext, bukan query sendiri. Selain
  // menghemat satu permintaan, nilainya otomatis ikut berubah begitu diganti
  // di halaman Pengaturan, tanpa perlu memuat ulang dashboard.
  ...
  const muat = useCallback(async () => {
    ...
    const [
      ...
      supabase.from('content_items').select('id,status,platform,pillar_id,scheduled_date,brief,assignee_id').eq('tenant_id', tenantId).limit(1000),
    ...
    setData({
      ...
      belumDitugaskan: items.filter((i) => !i.assignee_id && ACTIVE_STATUSES.includes(i.status)).length,
      // Beban kerja dihitung hanya dari pekerjaan yang masih berjalan.
      // Konten yang sudah tayang tidak lagi membebani siapa pun.
      bebanPerOrang: items
        .filter((i) => i.assignee_id && ACTIVE_STATUSES.includes(i.status))
        .reduce((acc, i) => { acc[i.assignee_id] = (acc[i.assignee_id] ?? 0) + 1; return acc }, {}),
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12 }}>
              ...
              <Metric
                ...
                value={data.belumDitugaskan}
                sub={data.belumDitugaskan > 0 ? 'belum ada penanggung jawab' : 'semua sudah ada pemiliknya'}
                warna={data.belumDitugaskan > 0 ? 'var(--warning)' : 'var(--success)'}
              />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14 }}>
              ...
              />
              <div className="card">
                <p style={{ fontSize: 13.5, fontWeight: 600, marginBottom: 3 }}>Beban kerja tim</p>
                <p style={{ fontSize: 11.5, color: 'var(--text-secondary)', marginBottom: 14 }}>
                  Konten yang masih berjalan, per orang.
                </p>
                {members.length === 0 ? (
                  <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>Belum ada anggota lain.</p>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {[...members]
                      .sort((a, b) => (data.bebanPerOrang[b.user_id] ?? 0) - (data.bebanPerOrang[a.user_id] ?? 0))
                      .map((m) => {
                        const jumlah = data.bebanPerOrang[m.user_id] ?? 0
                        const terbanyak = Math.max(1, ...Object.values(data.bebanPerOrang))
                        return (
                          <div key={m.user_id} style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                            <Avatar anggota={m} size={24} />
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                                <span style={{ fontSize: 12, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                  {namaAnggota(m)}
                                </span>
                                <span style={{ marginLeft: 'auto', fontSize: 12, fontWeight: 600 }}>{jumlah}</span>
                              </div>
                              <div className="bar-track" style={{ height: 5 }}>
                                <div
                                  className="bar-fill"
                                  style={{
                                    width: `${(jumlah / terbanyak) * 100}%`,
                                    background: jumlah === 0 ? 'var(--border)' : 'var(--accent)',
                                  }}
                                />
                              </div>
                            </div>
                ...
                <Link to="/team" className="btn btn-sm btn-block" style={{ textDecoration: 'none', marginTop: 14 }}>
                  Kelola tim
                </Link>
              </div>
